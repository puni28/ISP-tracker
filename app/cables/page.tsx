  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-4">
      <div className="flex items-center justify-between pt-2">
        ...
        <Link href="/cables/new" className="bg-blue-600 text-white text-sm px-4 py-2 rounded-lg">
          + Add
        </Link>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4">
        <Link href="/cables" className={`flex-shrink-0 text-xs px-3 py-1.5 rounded-full border ${!type && !status ? "bg-blue-600 border-blue-500 text-white" : "border-slate-600 text-slate-400"}`}>
          All
        </Link>
        {CABLE_TYPES.map((t) => (
      ...
      {cables.length === 0 ? (
        ...
        <div className="space-y-2">
          {cables.map((c) => {
            ...
            return (
              <Link key={c.id} href={`/cables/${c.id}`} className="flex items-center gap-3 bg-slate-800 rounded-xl p-4 border border-slate-700 hover:border-slate-500">
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <span className={`text-xs px-2 py-0.5 rounded border ${STATUS_COLORS[c.status]}`}>
                    {c.status}
                  </span>
                  {reading && (
