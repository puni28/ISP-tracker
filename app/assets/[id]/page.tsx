...
export default async function AssetDetailPage({ params }: { params: { id: string } }) {
  const asset = await prisma.networkAsset.findUnique({
    where: { id: params.id },
    include: {
      photos: { orderBy: { takenAt: "desc" }, take: 4 },
    ...
    <div className="p-4 max-w-lg mx-auto space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          ...
          className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-center text-sm hover:border-slate-500"
        >
          <div className="text-xl mb-1">📷</div>
          Photos ({asset.photos.length})
        </Link>
        <Link
          href={`/map?focus=${asset.id}`}
          className="bg-slate-800 border border-slate-700 rounded-xl p-3 text-center text-sm hover:border-slate-500"
        >
          <div className="text-xl mb-1">◉</div>
          View on Map
            ...
            {allCables.map((c) => {
              ...
              return (
                <Link
                  key={c.id}
                  href={`/cables/${c.id}`}
                  className="flex items-center gap-3 bg-slate-800 rounded-lg p-3 border border-slate-700 hover:border-slate-500"
                >
                  <span
                    className="w-3 h-3 rounded-full flex-shrink-0"
                    style={{ backgroundColor: CABLE_COLORS[c.type as CableType] }}
                  />
